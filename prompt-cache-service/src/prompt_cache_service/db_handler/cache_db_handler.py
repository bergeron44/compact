...
class CacheDbHandler(ABC, Generic[NS]):
    ...
    @abstractmethod
    def vote_entry(
        self, project_id: str, entry_id: str, vote_type: str
    ) -> tuple[int, int]:
    ...
    async def cache_prompt(
        ...
        try:
            ...
            self._logger.info(
                "Cached prompt: entry_id=%s, project_id=%s", entry.entry_id, project_id
            )
    ...
    async def lookup_prompt(
        ...
        candidates = self._pull_entry(
            project_id, key_embeddings, limit=limit * 3, threshold=threshold
        )
        ...
        self._logger.info(
            "Lookup hit: found %d matches (from %d candidates), project_id=%s",
            len(final_entries),
            len(candidates),
            project_id,
        )
...
class ChromaDbHandler(CacheDbHandler[chromadb.Collection]):
    ...
    def get_project_stats(self, project_id: str) -> dict:
        ...
        try:
            ...
            return {"total_entries": count, "total_hits": 0, "avg_compression": 0}
    ...
    def record_prompt_activity(
        ...
        collection.add(ids=[activity_id], documents=[query_text], metadatas=[meta])
        return activity_id
