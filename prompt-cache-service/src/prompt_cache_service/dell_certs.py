...
_DELL_CERT_MARKER = "# Dell Technologies PKI 2018 - installed by prompt-cache-service"


def _dell_certs_already_installed():
    """Check if Dell certificates are already in certifi bundle.

    Returns:
        bool: True if Dell certificates are already installed, False otherwise
    """
    cert_path = certifi.where()
    try:
        with open(cert_path, "r") as f:
            return _DELL_CERT_MARKER in f.read()
    except OSError:
        return False
...
def update_certifi_with_dell_certs():
    ...
    if _dell_certs_already_installed():
        logger.info(
            "Dell certificates already installed in certifi bundle, skipping update"
        )
        return
    else:
        logger.info("Dell certificates not installed in certifi bundle, updating...")
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            ...
            with open(cert_path, "a") as bundle:
                bundle.write("\n")
                bundle.write(_DELL_CERT_MARKER + "\n")
